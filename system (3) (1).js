...
async function savePatient() {
    ...
    try {
        if (editingPatientId) {
            ...
            // 建立搜尋關鍵字欄位，方便後續查詢
            try {
                patient.searchKeywords = generatePatientSearchKeywords(patient);
            } catch (e) {
                console.error('產生搜尋關鍵字失敗:', e);
            }
...
async function loadPatientListFromFirebase() {
    ...
    try {
        ...
        `;
        // 若有搜尋字串，改為使用關鍵字查詢以避免讀取整個集合
        if (searchTerm) {
            const matchedPatients = await searchPatientsByKeywords(searchTerm);
            // 使用查詢結果作為篩選列表
...
async function searchPatientsForRegistration() {
    ...
    try {
        // 使用關鍵字搜尋病人，避免一次載入整個集合
        const matchedPatients = await searchPatientsByKeywords(searchTerm);
        if (!Array.isArray(matchedPatients) || matchedPatients.length === 0) {
            resultsList.innerHTML = `
                <div class="p-4 text-center text-gray-500">
                    找不到符合條件的病人
                </div>
