...
(function() {
    ...
    const ACUPOINT_COORDS = {
        ...
        // 更新中府（手太陰肺經的募穴）座標。
        // 透過對原始穴位圖進行像素座標分析，找到中府穴的小圓點位於圖像的絕對座標約 (x=404px, y=328px)。
        // 以圖像尺寸 1116×768 計算相對座標，得到 x≈0.362、y≈0.427，藉此讓標記與實際穴位位置一致。
        // 更新中府座標：透過像素分析得到更精確的比例位置（約 x=0.3620、y=0.4271）。
        '中府': { x: 0.3620, y: 0.4271 },
        // 以下為其他穴位的座標，新增項目前請在前一項目後添加逗號
