...
/**
 * 根據搜尋字串從 Firestore 查詢病人資料。
 * 此函式使用 `searchKeywords` 欄位進行 `array-contains` 查詢，
 * 並限制返回筆數以避免一次讀取過多資料。若搜尋字串經過 trim
 * 並轉為小寫後為空，則直接回傳空陣列。
 *
 * @param {string} searchTerm 搜尋字串
 * @param {number} fetchLimit 最大返回筆數，預設為 50
 * @returns {Promise<Array>} 搜尋到的病人資料陣列
 */
async function searchPatientsByKeyword(searchTerm, fetchLimit = 50) {
    try {
        await waitForFirebaseDb();
        ...
        const limitNum = fetchLimit && fetchLimit > 0 ? fetchLimit : 50;
        // 建立查詢，利用 searchKeywords 欄位與 array-contains 搭配限制筆數
        let q = window.firebase.firestoreQuery(
            window.firebase.collection(window.firebase.db, 'patients'),
            window.firebase.where('searchKeywords', 'array-contains', term),
            window.firebase.orderBy('createdAt'),
            window.firebase.limit(limitNum)
        );
        const snapshot = await window.firebase.getDocs(q);
        const docs = [];
        snapshot.forEach((doc) => {
...
async function loadPatientListFromFirebase() {
    ...
    try {
        ...
        `;
        // 若有搜尋字串，透過關鍵字索引從 Firestore 直接查詢，避免讀取整個集合
        if (searchTerm) {
            // 決定一次查詢的筆數上限，默認取得多頁結果以供前端分頁
            const itemsPerPage = (paginationSettings && paginationSettings.patientList && paginationSettings.patientList.itemsPerPage) || 10;
            const fetchLimit = itemsPerPage * 5; // 預取五頁資料
            const matchedPatients = await searchPatientsByKeyword(searchTerm, fetchLimit);
            patientListFiltered = Array.isArray(matchedPatients) ? matchedPatients : [];
            // 按照建立時間排序（舊至新），以保持與分頁一致
            patientListFiltered.sort((a, b) => {
                const aTime = a.createdAt && a.createdAt.seconds ? a.createdAt.seconds * 1000 : (a.createdAt ? new Date(a.createdAt).getTime() : 0);
                const bTime = b.createdAt && b.createdAt.seconds ? b.createdAt.seconds * 1000 : (b.createdAt ? new Date(b.createdAt).getTime() : 0);
                return aTime - bTime;
            });
...
async function searchPatientsForRegistration() {
    ...
    try {
        // 根據搜尋字串直接使用關鍵字索引查詢病人。限制最大回傳筆數以避免讀取過多。
        const itemsPerPage = (paginationSettings && paginationSettings.patientList && paginationSettings.patientList.itemsPerPage) || 10;
        const fetchLimit = itemsPerPage * 5;
        const matchedPatients = await searchPatientsByKeyword(searchTerm, fetchLimit);
    ...
    try {
        ...
        async function confirmRegistration() {
            ...
            async function clearOldAppointments() {
                 ...
                 */
                try {
                    // 讀取所有**過期**掛號資料，僅查詢 appointmentTime 早於今日凌晨的紀錄
                    // 先計算今日凌晨時間（本地時區），並轉為 ISO 字串以供查詢使用
                    const now = new Date();
                    const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
                    const startIso = startOfToday.toISOString();
                    // 構建篩選過期掛號的 Realtime Database 查詢
...
class FirebaseDataManager {
    ...
    async addPatient(patientData) {
        // 新增病人資料時，加入 searchKeywords 欄位以支援高效搜尋
        if (!this.isReady) {
            showToast('數據管理器尚未準備就緒', 'error');
            return { success: false };
        }
        try {
            // 計算關鍵字索引。將姓名、電話、病歷號或身分證號的前綴拆分後存入陣列，
            // 方便後續透過 array-contains 查詢。若欄位不存在則跳過。
            const keywords = this.generateSearchKeywords(
                patientData && patientData.name,
                patientData && patientData.phone,
                patientData && patientData.patientNumber,
                patientData && patientData.idCard
            );
    ...
    async updatePatient(patientId, patientData) {
        try {
            // 計算新的關鍵字索引。如果提供資料中包含姓名、電話、病歷號或身分證號，
            // 將其拆分為前綴並生成 searchKeywords。若這些欄位不存在，將會產生空陣列，但此情況較少出現。
            const keywords = this.generateSearchKeywords(
                patientData && patientData.name,
                patientData && patientData.phone,
                patientData && patientData.patientNumber,
    ...
    /**
     * 產生搜尋關鍵字索引陣列。
     * 給定多個欄位值，將其轉為小寫並移除空白，再生成所有前綴組合。
     * 這些前綴將存入 searchKeywords 欄位，讓前綴匹配搜尋更為高效。
     * @param  {...any} fields  欄位值，可為姓名、電話、病歷號或身分證號
     * @returns {Array<string>} 去重後的關鍵字陣列
     */
    generateSearchKeywords(...fields) {
        const set = new Set();
        fields.forEach(field => {
            if (!field) return;
            let str = String(field).toLowerCase().replace(/\s+/g, '');
            if (!str) return;
            // 將完整字串加入集合
            set.add(str);
            // 產生所有前綴並加入集合
