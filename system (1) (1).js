...
async function renderPatientPackages(patientId) {
    ...
    try {
        ...
        // 按到期日排序所有套票，包含已用完的，以便使用者在使用完後仍可看到該紀錄
        const sortedPkgs = pkgs.slice().sort((a, b) => new Date(a.expiresAt) - new Date(b.expiresAt));
        container.innerHTML = sortedPkgs.map(pkg => {
            const exp = new Date(pkg.expiresAt);
            const now = new Date();
            // 判斷是否到期
            const expired = now > exp;
            // 沒有剩餘次數或已到期則停用按鈕，但仍顯示於列表中
            const disabled = expired || pkg.remainingUses <= 0;
            // 標記狀態徽章：到期或已用完
            const badge = expired
                ? '<span class="ml-2 text-xs text-white px-2 py-0.5 rounded bg-red-500">已到期</span>'
