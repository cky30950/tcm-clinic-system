...
function generateMedicalRecordNumber() {
    try {
        ...
        function setButtonLoading(button, loadingText) {
            ...
            // Save the original disabled state so we can restore it later. We use a string flag
            // because dataset stores only strings.
            ...
            /*
             * Show a spinning indicator while loading. If a loadingText is provided, append it
             * next to the spinner. Use a full border circle with one transparent side to make
             * the spinner more visible. The spinner inherits the button's current color via
             * border-current.
             */
            ...
            // Compose spinner and optional loading text
            const textHtml = loadingText ? `<span class="ml-2">${loadingText}</span>` : '';
            button.innerHTML = `<div class="inline-block animate-spin rounded-full h-4 w-4 border-2 border-current border-r-transparent"></div>${textHtml}`;
        }
        ...
        function clearButtonLoading(button) {
            ...
            // Restore the disabled state based on previously stored flag. If it was previously
            // disabled (string 'true'), keep it disabled; otherwise enable it.
...
async function selectPatientForRegistration(patientId) {
    ...
    try {
        ...
        async function confirmRegistration() {
            ...
            if (button) setButtonLoading(button, '掛號中...');
...
async function printConsultationRecord(consultationId, consultationData = null) {
    const button = typeof event !== 'undefined' && event ? event.currentTarget : null;
    if (button) setButtonLoading(button, '列印中...');
...
async function initializeSystemAfterLogin() {
    ...
    subscribeToAppointments();
        ...
        async function saveHerb() {
            const button = typeof event !== 'undefined' && event ? event.currentTarget : null;
            if (button) setButtonLoading(button, '儲存中...');
