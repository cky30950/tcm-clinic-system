...
// 為收費項目列表新增分頁設定，每頁顯示 10 筆資料
paginationSettings.billingItemsList = { currentPage: 1, itemsPerPage: 10 };
...
// ----------------------
// Billing Items 分頁與快取相關
// 與病人列表類似，透過游標控制器與快取物件管理收費項目的分頁讀取。
// billingItemsPagesCache[pageNumber] 儲存每頁已載入的收費項目；
// billingItemsPageCursors[pageNumber] 儲存該頁最後一筆文件的快照，用於下一頁查詢。
let billingItemsPagesCache = {};
let billingItemsPageCursors = {};

// 快取收費項目總數，用於分頁計算。讀取一次後會快取，除非強制刷新。
let billingItemsCountCache = null;
...
/**
 * 透過游標及 limit 方式分頁讀取收費項目資料。
 * 頁數從 1 開始計算。讀取後會快取該頁資料及最後一筆文件，以利下一頁查詢。
 * 當 forceRefresh 為 true 時，會清除所有分頁快取並重新讀取。
 *
 * @param {number} pageNumber 欲讀取的頁碼，從 1 起算
 * @param {boolean} forceRefresh 是否強制重新載入
 * @returns {Promise<Array>} 該頁收費項目資料陣列
 */
async function fetchBillingItemsPage(pageNumber = 1, forceRefresh = false) {
    // 參數檢查
    if (pageNumber < 1) pageNumber = 1;
    // 如果強制刷新，清空分頁快取與游標
    if (forceRefresh) {
        billingItemsPagesCache = {};
        billingItemsPageCursors = {};
    }
    // 若快取中已有對應頁資料，直接返回
    if (!forceRefresh && billingItemsPagesCache[pageNumber]) {
        return billingItemsPagesCache[pageNumber];
    }
    await waitForFirebaseDb();
    try {
        const pageSize = paginationSettings && paginationSettings.billingItemsList && paginationSettings.billingItemsList.itemsPerPage
            ? paginationSettings.billingItemsList.itemsPerPage
            : 10;
        // 建立基礎查詢：依照 createdAt 由舊至新排序
        let q = window.firebase.firestoreQuery(
        ...
        // 如果頁碼大於 1，且前一頁已記錄最後一筆文件，則以該文件為起點
        if (pageNumber > 1) {
            const prevCursor = billingItemsPageCursors[pageNumber - 1];
            if (prevCursor) {
                q = window.firebase.firestoreQuery(q, window.firebase.startAfter(prevCursor));
            }
        }
        ...
        if (docs.length > 0) {
            const lastDoc = snapshot.docs[snapshot.docs.length - 1];
            billingItemsPageCursors[pageNumber] = lastDoc;
        }
        // 快取本頁資料
        billingItemsPagesCache[pageNumber] = docs;
        return docs;
    } catch (error) {
        console.error('分頁讀取收費項目資料失敗:', error);
        return [];
    }
}

/**
 * 取得收費項目總數，透過 Firestore 聚合查詢 count()。
 * 結果將快取，以避免重複計算；除非 forceRefresh 為 true 才重新查詢。
 *
 * @param {boolean} forceRefresh 是否強制重新從 Firestore 讀取總數
 * @returns {Promise<number>} 收費項目數量
 */
async function getBillingItemsCount(forceRefresh = false) {
    // 若快取已存在且未要求強制刷新，直接返回快取值
    if (!forceRefresh && typeof billingItemsCountCache === 'number') {
        return billingItemsCountCache;
    }
    try {
        await waitForFirebaseDb();
        // 建立查詢（不指定排序或條件）
        ...
        const countSnap = await window.firebase.getCountFromServer(colRef);
        const count = countSnap.data().count;
        billingItemsCountCache = typeof count === 'number' ? count : 0;
        return billingItemsCountCache;
    } catch (error) {
        console.error('取得收費項目總數失敗:', error);
        return 0;
    }
}
...
function generateMedicalRecordNumber() {
    try {
        ...
        async function initBillingItems(forceRefresh = false) {
            ...
            await waitForFirebaseDb();
            try {
                // 透過分頁查詢讀取整個收費項目集合，使用 limit 與 startAfter 控制每次讀取數量
                // 每次讀取的筆數。使用較大的頁面大小以減少查詢次數。
                const pageSize = 100;
                let allItems = [];
                let lastDoc = null;
                let hasMore = true;
                // 依照 createdAt 遞增排序以避免重複讀取
                while (hasMore) {
                    let q = window.firebase.firestoreQuery(
                        window.firebase.collection(window.firebase.db, 'billingItems'),
                        window.firebase.orderBy('createdAt'),
                    ...
                    if (lastDoc) {
                        q = window.firebase.firestoreQuery(q, window.firebase.startAfter(lastDoc));
                    }
                    const snapshot = await window.firebase.getDocs(q);
                    const pageItems = [];
...
async function generatePatientNumberFromFirebase() {
    try {
        await waitForFirebaseDb();
        // 使用 Firestore 分頁查詢，按 patientNumber 由大到小排序，僅讀取第一筆資料
        const colRef = window.firebase.collection(window.firebase.db, 'patients');
        ...
        if (snapshot && !snapshot.empty) {
            ...
            const pNumber = docSnap.data().patientNumber;
            if (pNumber && /^P\d+$/.test(pNumber)) {
                const numeric = parseInt(pNumber.substring(1));
                if (!isNaN(numeric)) {
                    newNumber = numeric + 1;
                }
            }
