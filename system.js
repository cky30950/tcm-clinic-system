 ...
 * 並限制返回筆數以避免一次讀取過多資料。若搜尋字串為空，直接回傳空陣列。
 *
...
async function syncUserDataFromFirebase() {
    try {
        ...
        if (data && data.length > 0) {
            ...
            // 從 Firebase 讀取失敗或返回空陣列時，嘗試使用瀏覽器本地的用戶資料快取
            console.log('從 Firebase 取得的用戶資料為空或讀取失敗，使用本地數據');
            try {
                const localUsersJSON = localStorage.getItem('users');
                if (localUsersJSON) {
                    const localData = JSON.parse(localUsersJSON);
                    if (Array.isArray(localData) && localData.length > 0) {
                        users = localData;
                        console.log('已從本地存儲讀取用戶資料:', users.length, '筆');
                    }
                }
            } catch (e) {
                console.error('解析本地用戶資料失敗:', e);
            }
        }
    } catch (error) {
...
async function loadPatientListFromFirebase() {
    ...
    try {
        ...
        `;
        // 若有搜尋字串，先透過索引在伺服器端查詢資料；若查無結果再回退至前端篩選
        if (searchTerm) {
            ...
            let matchedPatients = await searchPatientsByKeyword(searchTerm, fetchLimit);
            if (!matchedPatients || matchedPatients.length === 0) {
                // 若索引搜尋無結果，讀取快取或伺服器資料並在前端篩選
                const allPatients = await fetchPatients();
                matchedPatients = Array.isArray(allPatients) ? allPatients.filter(patient =>
            ...
            patientListFiltered.sort((a, b) => {
                const aTime = a && a.createdAt && a.createdAt.seconds ? a.createdAt.seconds * 1000 : (a && a.createdAt ? new Date(a.createdAt).getTime() : 0);
                const bTime = b && b.createdAt && b.createdAt.seconds ? b.createdAt.seconds * 1000 : (b && b.createdAt ? new Date(b.createdAt).getTime() : 0);
                return aTime - bTime;
            });
...
async function searchPatientsForRegistration() {
    ...
    try {
        // 先透過 searchKeywords 在伺服器端查詢；若無結果再回退至前端篩選
        const itemsPerPage = (paginationSettings && paginationSettings.patientList && paginationSettings.patientList.itemsPerPage) || 10;
        const fetchLimit = itemsPerPage * 5;
        let matchedPatients = await searchPatientsByKeyword(searchTerm, fetchLimit);
        if (!matchedPatients || matchedPatients.length === 0) {
            // 若索引搜尋無結果，讀取全體病人資料並在前端進行篩選
            const allPatients = await fetchPatients();
            if (!allPatients || allPatients.length === 0) {
                resultsList.innerHTML = `
...
async function initializeSystemAfterLogin() {
    ...
    subscribeToAppointments();
        ...
        /*
         * editHerb 函式已移除：
         * 中藥材在前端僅提供瀏覽與新增功能，編輯操作由後端或管理介面處理。
         */
        ...
        /*
         * deleteHerbItem 函式已移除：
         * 中藥材與方劑的刪除功能已由後端管理，前端不再允許刪除操作。
         */
  ...
  try {
    ...
    console.error('刷新模板分類篩選下拉選單失敗:', e);
        ...
        function setupPersonalComboSearchAndFilter() {
          ...
          /*
           * addNewPrescriptionTemplate 函式已移除：
           * 醫囑模板的新增改由後端管理介面完成，前端僅提供檢視/使用功能。
           */

          /* duplicatePrescriptionTemplate 函式已移除（未使用） */

          /*
           * deletePrescriptionTemplate 函式已移除：
           * 前端不再支援刪除醫囑模板，避免意外刪除。
           */
          ...
          /*
           * addNewDiagnosisTemplate 函式已移除：
           * 診斷模板僅允許從後端管理介面建立，使用者不能透過前端介面直接新增。
           */

          /* duplicateDiagnosisTemplate 函式已移除（未使用） */

          /*
           * deleteDiagnosisTemplate 函式已移除：
           * 前端不再支援刪除診斷模板，避免意外操作。
           */
