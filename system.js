...
async function logout() {
    try {
        ...
        // 釋放聊天系統資源並更新線上狀態
        if (typeof destroyChatSystem === 'function') {
            try {
                destroyChatSystem();
            } catch (err) {
                console.error('銷毀聊天系統失敗:', err);
            }
        }
 ...
 * 聊天系統模組
 *
 * 此模組提供診所內部即時聊天功能。登入後會顯示一個位於右下角的聊天按鈕，
 * 點擊後打開聊天視窗，可即時查看線上用戶狀態與發送訊息。
 * 聊天內容與線上狀態透過 Firebase Realtime Database 儲存並同步。
 */
(function() {
  class ChatModule {
    constructor() {
    ...
    /**
     * 初始化聊天介面及資料監聽。
     * 此函式應該在 DOM 生成完成且 Firebase 初始化後調用。
     */
    async init() {
      // 取得 DOM 元素
      this.chatBtn = document.getElementById('chatToggleButton');
      this.chatPopup = document.getElementById('chatPopup');
      this.chatHeader = document.getElementById('chatHeader');
      this.chatCloseBtn = document.getElementById('chatCloseButton');
      this.userListEl = document.getElementById('chatUserList');
      this.messagesEl = document.getElementById('chatMessages');
      this.messageInput = document.getElementById('chatMessageInput');
      this.sendBtn = document.getElementById('chatSendButton');

      if (!this.chatBtn || !this.chatPopup) {
        console.warn('聊天按鈕或彈窗不存在，初始化聊天系統失敗');
        return;
      }

      // 綁定事件
      this.chatBtn.addEventListener('click', () => this.togglePopup());
      if (this.chatCloseBtn) {
        this.chatCloseBtn.addEventListener('click', () => this.closePopup());
      }
      ...
      if (!window.firebase || !window.firebase.rtdb) {
        console.warn('Firebase Realtime Database 尚未就緒，聊天功能無法使用');
        return;
      }
      // 監聽 presence（線上狀態）
      this.presenceRef = window.firebase.ref(window.firebase.rtdb, 'presence');
      this.presenceCallback = (snapshot) => {
        this.presenceData = snapshot.val() || {};
        this.renderUserList();
      };
      window.firebase.onValue(this.presenceRef, this.presenceCallback);
      // 監聽聊天訊息
      this.messagesRef = window.firebase.ref(window.firebase.rtdb, 'chat/messages');
      this.messagesCallback = (snapshot) => {
        const data = snapshot.val() || {};
        // 將物件轉成陣列並依 timestamp 排序
        const arr = Object.values(data).filter(item => item && typeof item.timestamp !== 'undefined');
        arr.sort((a, b) => {
      ...
      window.firebase.onValue(this.messagesRef, this.messagesCallback);
    }
    ...
    setCurrentUser(user) {
      ...
      // 設置 beforeunload 事件，以確保關閉視窗或刷新時記錄離線狀態
      if (!this.beforeUnloadHandler) {
        this.beforeUnloadHandler = () => {
          try {
            this.updatePresenceStatus(false);
          } catch (e) {
            console.error('更新離線狀態失敗', e);
          }
        };
    ...
    /**
     * 更新使用者的線上狀態。
     * @param {boolean} online 當前是否在線
     */
    async updatePresenceStatus(online) {
      if (!this.currentUser || !window.firebase || !window.firebase.rtdb) return;
      try {
        const refPath = window.firebase.ref(window.firebase.rtdb, `presence/${this.currentUser.id}`);
        const payload = {
          id: this.currentUser.id,
          username: this.currentUser.username || '',
          displayName: (typeof getUserDisplayName === 'function') ? getUserDisplayName(this.currentUser) : (this.currentUser.username || ''),
          online: !!online,
          timestamp: Date.now()
        };
        await window.firebase.set(refPath, payload);
      } catch (err) {
        console.error('更新線上狀態時發生錯誤', err);
      }
    }
     ...
     */
    async sendMessage() {
      if (!this.messageInput || !window.firebase || !window.firebase.rtdb) return;
      const text = this.messageInput.value.trim();
      if (!text) return;
      if (!this.currentUser) {
        console.warn('尚未設置當前使用者，無法發送訊息');
        return;
      }
      // 建立訊息物件
      ...
      try {
        const msgRef = window.firebase.ref(window.firebase.rtdb, `chat/messages/${messageObj.id}`);
        await window.firebase.set(msgRef, messageObj);
    ...
    /**
     * 渲染用戶列表，依據 Firebase presence 狀態顯示線上/離線指示。
     */
    renderUserList() {
      ...
      // 計算線上用戶數量並更新標題列顯示
      const countSpan = document.getElementById('chatOnlineCount');
      if (countSpan) {
        const onlineCount = Object.values(presence).filter(p => p && p.online).length;
        countSpan.textContent = onlineCount > 0 ? `${onlineCount} 人在線` : '';
      }
      // 生成列表內容
      let html = '';
      userArr.forEach(u => {
        const status = presence && presence[u.id] && presence[u.id].online;
        const indicatorClass = status ? 'bg-green-400' : 'bg-gray-400';
        const displayName = (typeof getUserDisplayName === 'function') ? getUserDisplayName(u) : (u.username || '');
        html += `<div class="flex items-center space-x-2 p-1 rounded hover:bg-gray-100">
  <div class="w-2 h-2 rounded-full ${indicatorClass}"></div>
  <span class="text-sm truncate">${this.escapeHtml(displayName)}</span>
</div>`;
      });
    ...
    renderMessages() {
      ...
      this.messages.forEach(msg => {
        const isMe = this.currentUser && String(msg.userId) === String(this.currentUser.id);
        const timeStr = new Date(msg.timestamp).toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' });
        const displayName = msg.displayName || msg.username || '';
        const safeText = this.escapeHtml(msg.text || '');
        if (isMe) {
          html += `<div class="flex items-start space-x-2 justify-end">
  <div class="flex-1 flex flex-col items-end">
    <div class="flex items-center space-x-1 mb-1">
      <span class="text-xs text-gray-500">${timeStr}</span>
      <span class="font-medium text-xs text-gray-700">${this.escapeHtml(displayName)}</span>
    </div>
    <div class="bg-blue-500 text-white rounded-2xl rounded-tr-md px-3 py-1 max-w-xs break-words">
  ...
  <div class="w-6 h-6 ${colorClass} rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">${this.escapeHtml(initial)}</div>
  <div class="flex-1">
    <div class="flex items-center space-x-1 mb-1">
      <span class="font-medium text-xs text-gray-700">${this.escapeHtml(displayName)}</span>
      <span class="text-xs text-gray-500">${timeStr}</span>
    </div>
    <div class="bg-gray-100 rounded-2xl rounded-tl-md px-3 py-1 max-w-xs break-words">
      <p class="text-sm text-gray-800">${safeText}</p>
    </div>
  </div>
</div>`;
        }
     ...
     */
    initDrag() {
      if (!this.chatHeader || !this.chatPopup) return;
      // mousedown 觸發拖曳
      this.chatHeader.addEventListener('mousedown', (e) => {
        ...
        // 將固定位置改為絕對位置
        this.chatPopup.style.left = `${rect.left}px`;
        this.chatPopup.style.top = `${rect.top}px`;
        ...
        const mouseMove = (ev) => {
          ...
          this.chatPopup.style.left = `${x}px`;
          this.chatPopup.style.top = `${y}px`;
        };
        const mouseUp = () => {
          this.isDragging = false;
    ...
    /**
     * HTML escape utility to prevent XSS in messages or names.
     * @param {string} text
     * @returns {string}
     */
    escapeHtml(text) {
    ...
    /**
     * 解除監聽並清理資源。在登出時或需要重新初始化時調用。
     */
    async destroy() {
      // 移除 Realtime Database 監聽
      try {
        if (this.presenceRef && this.presenceCallback) {
          window.firebase.off(this.presenceRef, 'value', this.presenceCallback);
        }
        if (this.messagesRef && this.messagesCallback) {
          window.firebase.off(this.messagesRef, 'value', this.messagesCallback);
        }
      } catch (err) {
        console.error('移除聊天監聽器失敗:', err);
      }
      // 更新為離線狀態
      try {
        await this.updatePresenceStatus(false);
      } catch (err) {
        console.warn('更新離線狀態失敗:', err);
      }
      // 移除 beforeunload 事件
      if (this.beforeUnloadHandler) {
      ...
      // 清理 DOM 事件
      if (this.chatBtn) {
        this.chatBtn.removeEventListener('click', () => this.togglePopup());
      }
      if (this.chatCloseBtn) {
        this.chatCloseBtn.removeEventListener('click', () => this.closePopup());
      }
      if (this.messageInput) {
        this.messageInput.removeEventListener('input', () => this.updateSendButton());
      }
      if (this.sendBtn) {
  ...
  /**
   * 初始化聊天系統。如果尚未建立實例則建立並初始化，再更新當前使用者。
   */
  window.initChatSystem = function() {
    ...
    if (!chatModuleInstance) {
      chatModuleInstance = new ChatModule();
      // 初始化並忽略異步錯誤，於控制台輸出
      chatModuleInstance.init().catch((err) => {
        console.error('聊天模組初始化失敗:', err);
      });
    }
    // 更新目前使用者
    if (chatModuleInstance && typeof chatModuleInstance.setCurrentUser === 'function' && currentUserData) {
      chatModuleInstance.setCurrentUser(currentUserData);
      chatModuleInstance.showButton();
    }
  };
   ...
   * 銷毀聊天系統並清理資源。
   */
  window.destroyChatSystem = function() {
    if (chatModuleInstance && typeof chatModuleInstance.destroy === 'function') {
      chatModuleInstance.destroy().catch(() => {});
    }
