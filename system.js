...
async function loadPatientListFromFirebase() {
    ...
    if (!tbody) return;
    // 嘗試同時支援舊 ID (searchPatient) 及新版 ID (patientSearchInput) 的搜尋欄位。
    // 部分頁面可能使用不同的元素 ID，如果僅查詢其中一個會導致搜尋無法運作。
    const searchInput = document.getElementById('searchPatient') || document.getElementById('patientSearchInput');
...
function renderPatientListTable(pageChange = false) {
    ...
    if (!Array.isArray(patientListFiltered) || patientListFiltered.length === 0) {
        // 嘗試從多個可能的搜尋欄位取得搜尋字串
        const searchTermEl = document.getElementById('searchPatient') || document.getElementById('patientSearchInput');
        const searchTerm = searchTermEl ? String(searchTermEl.value || '').toLowerCase() : '';
        tbody.innerHTML = `
            <tr>
                <td colspan="6" class="px-4 py-8 text-center text-gray-500">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 嘗試支援多個可能的搜尋欄位 ID (searchPatient 與 patientSearchInput)。
    const searchInput = document.getElementById('searchPatient') || document.getElementById('patientSearchInput');
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            // 搜尋時重置分頁至第一頁
