        ...
        // 為避免 XSS，建立替換後的 content
        {
            ...
            try {
                ...
                if (Array.isArray(pkgs) && pkgs.length > 0) {
                    // 依到期日排序，以便即將到期的套票先顯示
                    const sortedPkgs = pkgs.slice().sort((a, b) => new Date(a.expiresAt) - new Date(b.expiresAt));
                    packageStatusHtml = `
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <div class="flex justify-between items-center mb-4">
                    ...
                    <div class="space-y-2">
                        ${sortedPkgs.map(pkg => {
                            ...
                            return `
                            <div class="flex items-center justify-between bg-purple-50 border border-purple-200 rounded p-2">
                                <div>
                                    <div class="font-medium text-purple-900">${safePkgName}</div>
                                    <div class="text-xs text-gray-600">${safeStatusText}</div>
                                </div>
                                <div class="text-sm text-gray-800">${remainingUses}${totalUses !== '' ? '/' + totalUses : ''}</div>
                            </div>
                            `;
                        }).join('')}
                    </div>
