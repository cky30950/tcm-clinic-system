...
/*
 * Firebase Authentication 狀態監聽器
 *
 * 為了即時感知使用者登入狀態的變更，加入 onAuthStateChanged 監聽。當使用者的憑證過期
 * 或者在其他裝置登出時，Auth 的 currentUser 會變成 null。此時我們切換介面回登入頁，
 * 避免未授權的使用者繼續操作系統。
 */
(function() {
  // 使用立即呼叫的 async 函式等待 Firebase 初始化完成
  (async () => {
    try {
      // 等待 Firebase 全域對象初始化
      ...
      // 確保 onAuthStateChanged 函式存在
      if (window.firebase && typeof window.firebase.onAuthStateChanged === 'function' && window.firebase.auth) {
        window.firebase.onAuthStateChanged(window.firebase.auth, (user) => {
          ...
          if (!user) {
            try {
              ...
              if (mainSystemEl) mainSystemEl.classList.add('hidden');
            } catch (_e) {
              // 忽略切換介面時的錯誤
            }
          }
      ...
      console.error('初始化 Firebase Auth 狀態監聽器失敗:', e);
    }
  })();
})();
