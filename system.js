...
(function() {
  ...
  function updateNetworkStatus() {
    const online = navigator.onLine && window.firebaseConnected;
    // 當離線且先前未處於離線狀態時處理
    if (!online && !wasOffline) {
      wasOffline = true;
      // 顯示覆蓋層
      showOfflineOverlay('網路中斷，請檢查網路');
      // 提示訊息
      if (typeof window.showToast === 'function') {
        window.showToast('網路中斷，請檢查網路', 'error');
      }
      ...
      hideOfflineOverlay();
      if (typeof window.showToast === 'function') {
        window.showToast('網路已恢復，請重新操作', 'success');
      }
      // 網路恢復後可在此處進行資料重新載入或其他邏輯
    }
  }
